...
class Kdecay:
    ...
    def __init__(self):
        ...
        # magnitude in kaon rest frame 
        self.e_pion_kaon_rest_frame = self.m_kaon / 2.
        self.p_pion_kaon_rest_frame = np.sqrt(self.e_pion_kaon_rest_frame**2 - self.m_pion**2)
    ...
    def getdecay(self):
        ...
        # get the location of the kaon's decay point. 
        self.get_decay_vertex()
        
        # now generate the kaon decay in the kaon's rest frame
        self.decay_the_kaon_in_its_cm()
        
        # now transform the pion momenta to the lab frame
    ...
    def get_decay_vertex(self):
        ...
        while not we_have_a_decay_vertex:
            ...
            relative_probability = np.exp(-trial_z / (self.kaon_beta * self.kaon_gamma * self.clight * self.tau_kaon))
    ...
    def decay_the_kaon_in_its_cm(self):
        ...
        # momentum in the kaon rest frame.
    ...
    def transform_to_lab(self):
        ...
        # lorentz transform the z components
        self.p_piplus[2] = self.kaon_gamma * (self.p_piplus_k_rest_frame[2] + self.kaon_beta * self.e_pion_kaon_rest_frame)

        self.p_piminus[2] = self.kaon_gamma * (self.p_piminus_k_rest_frame[2] + self.kaon_beta * self.e_pion_kaon_rest_frame)

        # lorentz transform the energies
        self.e_piplus = self.kaon_gamma * (self.e_pion_kaon_rest_frame + self.kaon_beta * self.p_piplus_k_rest_frame[2])

        self.e_piminus = self.kaon_gamma * (self.e_pion_kaon_rest_frame + self.kaon_beta * self.p_piminus_k_rest_frame[2])