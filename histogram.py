...
class histo:
    ...
    def hfill(self, xvalue, yvalue = np.nan):
        ...
        if xvalue >= self.xmin and xvalue <= self.xmax:
            ...
            ixbin = math.floor((xvalue - self.xmin) / self.dx)
        ...
        if self.htype == 1 and ixbin >= 0:
            self.binpop[ixbin] = self.binpop[ixbin] + 1
            self.ntot = self.ntot + 1
    
        if self.htype == 2 and ixbin >= 0 and iybin >= 0:
            self.binpop[ixbin, iybin] = self.binpop[ixbin, iybin] + 1
            self.ntot = self.ntot + 1
        ...
        if self.ntot > 0:
            ...
            self.xrms = np.sqrt(mean_square - self.xmean * self.xmean)
    
            if self.htype == 2:
    
                self.ymean = self.ysum / self.ntot
                mean_square = self.ysumsq / self.ntot
                self.yrms = np.sqrt(mean_square - self.ymean * self.ymean)
    ...
    def hprint(self, rows = 0, columns = 0, which_pane = 0):
    ...
    def hdraw(self,  rows, columns, which_pane, figure_handle):
        ...
        if self.htype == 1 or number_dimensions == 1:
            ...
            plt.text(0.02, 0.98, statistics_message, \
